...
int main(int argc, char const **argv) {
    ...
    if (!clingo_control_configuration(ctl, &conf)) {
        goto error;
    }
    if (!clingo_configuration_root(conf, &root_key)) {
        goto error;
    }

    // configure to enumerate all models
    if (!clingo_configuration_map_at(conf, root_key, "solve.models", &sub_key)) {
        goto error;
    }
    if (!clingo_configuration_value_set(conf, sub_key, "0")) {
        goto error;
    }

    // configure the first solver to use the berkmin heuristic
    if (!clingo_configuration_map_at(conf, root_key, "solver", &sub_key)) {
        goto error;
    }
    if (!clingo_configuration_array_at(conf, sub_key, 0, &sub_key)) {
        goto error;
    }
    if (!clingo_configuration_map_at(conf, sub_key, "heuristic", &sub_key)) {
        goto error;
    }
    if (!clingo_configuration_value_set(conf, sub_key, "berkmin")) {
        goto error;
    }
    ...
    if (!clingo_control_add(ctl, "base", NULL, 0, "a :- not b. b :- not a.")) {
        goto error;
    }
