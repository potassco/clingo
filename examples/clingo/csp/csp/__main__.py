...
class Application(object):
    ...
    def __init__(self):
        ...
        self.config = AppConfig(self._propagator.config)
    ...
    def register_options(self, options):
        ...
        # translation
        options.add_flag(group, "shift-constraints",
        ...
        options.add_flag(group, "literals-only",
                    "Only create literals during translation but no clauses [{}]".format(self._flag_str(conf.literals_only)),
                    conf.literals_only)
        options.add(group, "translate-pb",
                    "Restrict translation to <n> literals per pb constraint [{}]\n".format(conf.weight_constraint_limit),
                    self._parse_int(conf, "weight_constraint_limit", min_value=0), argument="<n>")

        # propagation
        options.add(group, "refine-reasons",
                    dedent("""\
        ...
        options.add(group, "refine-introduce",
                    dedent("""\
        ...
        options.add(group, "propagate-chain",
                    dedent("""\
        ...
        # hidden/debug
        options.add(group, "min-int,@2",
                    "Minimum integer [{}]".format(conf.min_int),
                    self._parse_int(conf, "min_int"), argument="<i>")
        options.add(group, "max-int,@2",
                    "Maximum integer [{}]".format(conf.max_int),
                    self._parse_int(conf, "max_int"), argument="<i>")
        options.add_flag(group, "check-solution,@2",
                    "Debug option to verify solutions [{}]".format(self._flag_str(conf.check_solution)),
                    conf.check_solution)
        options.add_flag(group, "check-state,@2",
                    "Debug option to verify state consistency [{}]".format(self._flag_str(conf.check_state)),
                    conf.check_state)
