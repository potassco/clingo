...
public:
	...
	ConstraintType type() const { return Constraint_t::static_constraint; }
...
public:
	...
	typedef PodVector<WeightSum>::type WeightSumVec;
	typedef WeightLitVec Rule;
	...
	WeightSum getSum(uint32 level) const {
		assert(level < minRules_.size());
		return minRules_[level]->sum_;
	}
	 ...
	 */
	void minimize(Solver& s, const Rule& literals, bool inHeu = false);

	const Rule& minRule(uint32 i) const {
		assert(i < numRules());
		return minRules_[i]->lits_;
	}
	...
	//! called by enumerator
	bool backpropagate(Solver& s);

	//! integrates the given optimum in the current search
	/*!
	 * \param s The solver in which the search takes place
	 * \param p The active post propagator or 0
	 * \param newOpt The new optimum to be integrated
	 * \pre Either p == 0 or p is active, i.e. the function is called
	 * in the context of p.
	 * \pre newOpt is strictly less than the optimum stored in *this
	 */
	bool integrateNewOptimum(Solver& s, PostPropagator* p, const WeightSumVec& newOpt);
	...
	ConstraintType type() const {
		return Constraint_t::static_constraint;
	}
