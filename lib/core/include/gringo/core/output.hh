...
class OutputLit {
  public:
    ...
    void lit(Sign sign, SymbolRef sym) { do_lit(sign, sym); }
    void boolean(bool value) { do_boolean(value); }
    void cond_lit(size_t uid) { do_cond_lit(uid); }

  private:
    virtual void do_lit(Sign sign, SymbolRef sym) = 0;
...
class OutputStm {
  public:
    ...
    void fact(SymbolRef sym) { do_fact(sym); }

    auto body() -> OutputLit & { return do_body(); }
    void rule(std::optional<SymbolRef> head) { do_rule(head); }
  ...
  private:
    ...
    virtual void do_fact(SymbolRef sym) = 0;

    virtual auto do_body() -> OutputLit & = 0;
    virtual void do_rule(std::optional<SymbolRef> head) = 0;
