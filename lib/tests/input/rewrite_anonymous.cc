...
TEST_CASE("rewrite_anonymous_head") {
    // disjunctions
    REQUIRE(rewrite_statement("p(X,_): q(X,_,*).") == "p(X,__Projected_0): q(X,__Projected_1,*).");
    REQUIRE(rewrite_statement("not p(X,_,*): not r(X,_,*).") == "not p(X,__Projected_0,*): not r(X,__Projected_1,*).");
    REQUIRE(rewrite_statement("f(X,_)<g(X,_).") == "f(X,__Projected_0)<g(X,__Projected_1).");
...
TEST_CASE("rewrite_anonymous_body") {
    // conjunctions
    REQUIRE(rewrite_statement(":- p(X,_,*): q(X,_,*).") == " :- p(X,__Projected_0,*): q(X,__Projected_1,*).");
    REQUIRE(rewrite_statement(":- not p(X,_,*): not r(X,_,*).") ==
...
TEST_CASE("rewrite_anonymous_statement") {
    ...
    REQUIRE(rewrite_statement("#minimize { Y@Z: not p(X), not p(_) }.") ==
            "#minimize { Y@Z: not p(X), not p(__Projected_0) }.");
    REQUIRE(rewrite_statement(":~ not p(X), not p(_). [Y]") == " :~ not p(X); not p(__Projected_0). [Y]");
    REQUIRE(rewrite_statement("#show p(X,Y): not p(X); not q(_).") == "#show p(X,Y): not p(X); not q(__Projected_0).");
    REQUIRE(rewrite_statement("#show p/2.") == "#show p/2.");
    REQUIRE(rewrite_statement("#project p(X): not q(Y), not q(_).") ==
            "#project p(X): not q(Y); not q(__Projected_0).");
    ...
    REQUIRE(rewrite_statement("#external p(X): not q(Y), not q(_). [Z]") ==
            "#external p(X): not q(Y); not q(__Projected_0). [Z]");
    REQUIRE(rewrite_statement("#edge (X,X): not q(Y), not q(_).") == "#edge (X,X): not q(Y); not q(__Projected_0).");
    REQUIRE(rewrite_statement("#heuristic p(X): not q(Y), not q(_). [U,V]") ==
            "#heuristic p(X): not q(Y); not q(__Projected_0). [U,V]");
