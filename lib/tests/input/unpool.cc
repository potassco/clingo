...
template <class T> auto unpool_str(Logger &log, std::optional<T> value, char const *sep = ", ") -> std::string {
    if (value) {
        auto unpooled = unpool(log, value.value());
...
TEST_CASE("unpool_statement") {
    Logger log;
    // rule
    REQUIRE(unpool_str(log, parse_statement("a(1;2) :- b(3;4), b(5,6)."), " ") == "[a(1) :- b(3); b(5,6)."
                                                                                  " a(1) :- b(4); b(5,6)."
                                                                                  " a(2) :- b(3); b(5,6)."
                                                                                  " a(2) :- b(4); b(5,6).]");
    REQUIRE(unpool_str(log, parse_statement("a(1;2) :- b(3;4), b(5;6)."), " ") == "[a(1) :- b(3); b(5)."
                                                                                  " a(1) :- b(4); b(5)."
                                                                                  " a(1) :- b(3); b(6)."
                                                                                  " a(1) :- b(4); b(6)."
                                                                                  " a(2) :- b(3); b(5)."
                                                                                  ...
                                                                                  " a(2) :- b(4); b(6).]");
    // minimize
    REQUIRE(unpool_str(log, parse_statement("#minimize { (1;2)@(2;3),(4;5): p(1;2) }.")) ==
            "[#minimize { "
            ...
            "}.]");
    // weak constraint
    REQUIRE(unpool_str(log, parse_statement(":~ p(1;2). [(1;2)@(2;3),(4;5)]"), " ") ==
    ...
    // show
    REQUIRE(unpool_str(log, parse_statement("#show (1;2): p(1;2)."), " ") == "[#show 1: p(1). #show 1: p(2)."
                                                                             " #show 2: p(1). #show 2: p(2).]");
    // project
    REQUIRE(unpool_str(log, parse_statement("#project q(1;2): p(1;2)."), " ") ==
            "[#project q(1): p(1). #project q(1): p(2)."
            " #project q(2): p(1). #project q(2): p(2).]");
    // edge
    REQUIRE(unpool_str(log, parse_statement("#edge ((a;b),(c;d)) : p(1;2)."), " ") ==
            "[#edge (a,c): p(1). #edge (a,d): p(1)."
            " #edge (b,c): p(1). #edge (b,d): p(1)."
            " #edge (a,c): p(2). #edge (a,d): p(2)."
            " #edge (b,c): p(2). #edge (b,d): p(2).]");
    REQUIRE(unpool_str(log, parse_statement("#heuristic p(1;2). [(1;2)@(3;4),(5;6)]"), " ") ==
    ...
    REQUIRE(unpool_str(log, parse_statement("#const x=(1)."), "") == "[#const x=1. [default]]");

    // local <-> global
    // TODO: catch errors!!!
    REQUIRE(unpool_str(log, parse_statement(":- p(X): q(X;Y)."), " ") == "[ :- p(X): q(X); p(X): q(Y).]");
    REQUIRE_THROWS(unpool_str(log, parse_statement(":- p(X;Y): q(X)."), " "));
    REQUIRE_THROWS(unpool_str(log, parse_statement(":- p(X;Y): q(X;Y)."), " "));
    REQUIRE_THROWS(unpool_str(log, parse_statement(":- p(X): q(Y); r(X;Y)."), " "));
}
