...
auto SolveHandle::get() -> SolveResult {
    ...
    handle_error(clingo_solve_handle_get(hnd_, &res), exception_());
    return {res};
}

void SolveHandle::cancel() {
    auto release = py::gil_scoped_release{};
    handle_error(clingo_solve_handle_cancel(hnd_), exception_());
}

void SolveHandle::resume() {
    handle_error(clingo_solve_handle_resume(hnd_), exception_());
}

auto SolveHandle::model() -> std::optional<Model> {
    auto release = py::gil_scoped_release{};
    auto const *mdl = static_cast<clingo_model_t const *>(nullptr);
    handle_error(clingo_solve_handle_model(hnd_, &mdl), exception_());
    return mdl != nullptr ? std::make_optional<Model>(mdl) : std::nullopt;
}

auto SolveHandle::last() -> std::optional<Model> {
    auto const *mdl = static_cast<clingo_model_t const *>(nullptr);
    handle_error(clingo_solve_handle_last(hnd_, &mdl), exception_());
    return mdl != nullptr ? std::make_optional<Model>(mdl) : std::nullopt;
}
...
auto SolveHandle::core() -> std::span<clingo_literal_t const> {
    ...
    auto size = size_t{0};
    handle_error(clingo_solve_handle_core(hnd_, &lits, &size), exception_());
    return {lits, size};
}
...
auto SolveHandle::wait(std::optional<double> timeout) -> bool {
    auto release = py::gil_scoped_release{};
    bool result = false;
    handle_error(clingo_solve_handle_wait(hnd_, timeout ? *timeout : -1, &result), exception_());
    return result;
}
...
void SolveHandle::close() {
    if (hnd_ != nullptr) {
        auto release = py::gil_scoped_release{};
        handle_error(clingo_solve_handle_close(std::exchange(hnd_, nullptr)), exception_());
    }
}
