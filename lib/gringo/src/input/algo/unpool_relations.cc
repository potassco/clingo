        ...
        for (auto const &elem : lit.elems_) {
            ...
            if (res_lit || res_cond) {
                res_elems.replace(HeadAggregate::Element{
                    elem.loc(), elem.tuple_, std::move(res_lit).value_or(elem.lit_), std::move(res_cond).value()});
            } else {
                res_elems.keep();
            }
        }
        if (res_elems) {
            return HeadAggregate{lit.loc(), lit.lhs_, lit.fun_, *std::move(res_elems), lit.rhs_};
        }
        return std::nullopt;
    }
    ...
    void operator()(BodyAggregate const &lit) const {
        ...
        for (auto const &elem : lit.elems_) {
            if (auto res_cond = unpool_conjunctive(elem.cond_); res_cond) {
                res_elems.replace(BodyAggregate::Element{elem.loc(), elem.tuple_, *std::move(res_cond)});
            } else {
                res_elems.keep();
            }
        }
        ...
        if (lit.sign_ == Sign::none && has_assign && lit.rhs_) {
            body.remove();
            if (lit.lhs_) {
                body.append(BodyAggregate{lit.loc(), lit.sign_, lit.lhs_, lit.fun_, *res_elems, std::nullopt});
            }
            body.append(BodyAggregate{lit.loc(), lit.sign_, std::make_pair(lit.rhs_->second, lit.rhs_->first), lit.fun_,
                                      *std::move(res_elems), std::nullopt});
        } else if (res_elems) {
            body.replace(BodyAggregate{lit.loc(), lit.sign_, lit.lhs_, lit.fun_, *std::move(res_elems), lit.rhs_});
        } else {
            body.keep();
        }
    }
...
struct UnpoolHeadBody {
    ...
    auto operator()(HeadAggregate const &lit) const -> std::optional<HeadLiteralVec> {
        auto unpool_elem = [](HeadAggregate::Element const &elem) {
            auto build = [&elem](auto cond) {
                return HeadAggregate::Element{elem.loc(), elem.tuple_, elem.lit_, std::move(cond)};
            };
            return unpool_crossproducts(build, unpool_disjunctive, elem.cond_);
        };
        auto res_elems = unpool_union(lit.elems_, unpool_elem);
        if (res_elems) {
            return Util::make_vec<HeadLiteral>(
                HeadAggregate{lit.loc(), lit.lhs_, lit.fun_, *std::move(res_elems), lit.rhs_});
        }
        return std::nullopt;
    }
    ...
    auto operator()(BodyAggregate const &lit) const -> std::optional<std::vector<BodyLiteral>> {
        auto unpool_elem = [](BodyAggregate::Element const &elem) {
            auto build = [&elem](auto cond) {
                return BodyAggregate::Element{elem.loc(), elem.tuple_, std::move(cond)};
            };
            return unpool_crossproducts(build, unpool_disjunctive, elem.cond_);
        };
        auto res_elems = unpool_union(lit.elems_, unpool_elem);
        if (res_elems) {
            return Util::make_vec<BodyLiteral>(
                BodyAggregate{lit.loc(), lit.sign_, lit.lhs_, lit.fun_, *std::move(res_elems), lit.rhs_});
        }
        return std::nullopt;
    }
...
struct UnpoolStatement {
    ...
    auto operator()(Rule const &stm) const -> std::optional<StatementVec> {
        ...
        auto unpool = UnpoolHeadBody{ctx};
        if (head || body) {
            auto shifted = Rule{stm.loc(), std::move(head).value_or(stm.head_), std::move(body).value()};
    ...
    auto operator()(StatementWeakConstraint const &stm) const -> std::optional<StatementVec> {
        auto build = [&stm](auto body) -> Statement {
            return StatementWeakConstraint{stm.loc(), std::move(body), stm.tuple_};
        };
        return rewrite_with_body(build, stm.body_);
    }

    auto operator()(StatementShow const &stm) const -> std::optional<StatementVec> {
        auto build = [&stm](auto body) -> Statement { return StatementShow{stm.loc(), stm.term_, std::move(body)}; };
        return rewrite_with_body(build, stm.body_);
    }
    ...
    auto operator()(StatementProject const &stm) const -> std::optional<StatementVec> {
        auto build = [&stm](auto body) -> Statement { return StatementProject{stm.loc(), stm.term_, std::move(body)}; };
        return rewrite_with_body(build, stm.body_);
    }
    ...
    auto operator()(StatementExternal const &stm) const -> std::optional<StatementVec> {
        auto build = [&stm](auto body) -> Statement {
            return StatementExternal{stm.loc(), stm.term_, std::move(body), stm.type_};
        };
        return rewrite_with_body(build, stm.body_);
    }

    auto operator()(StatementEdge const &stm) const -> std::optional<StatementVec> {
        auto build = [&stm](auto body) -> Statement { return StatementEdge{stm.loc(), stm.edges_, std::move(body)}; };
        return rewrite_with_body(build, stm.body_);
    }

    auto operator()(StatementHeuristic const &stm) const -> std::optional<StatementVec> {
        auto build = [&stm](auto body) -> Statement {
            return StatementHeuristic{stm.loc(), stm.atom_, std::move(body), stm.type_, stm.prio_, stm.mod_};
        };
        return rewrite_with_body(build, stm.body_);
    }
