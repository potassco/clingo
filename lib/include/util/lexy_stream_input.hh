...
template <typename StreamBuffer> class StreamReader {
  public:
    ...
    explicit StreamReader(StreamBuffer &buffer, size_t id = 0) : buffer_(&buffer), id_{id} {}
    ...
    void bump() noexcept { ++id_; }

    /// Get an iterator to the current position of the reader.
    auto position() const noexcept { return iterator(*buffer_, id_); }

    /// Set the current position of the reader.
    void set_position(iterator new_pos) noexcept { id_ = new_pos.offset(); }

    /// Discard all bytes before the current position.
    void discard_before() { buffer_->discard(id_); }
...
class StreamBuffer {
  public:
    ...
    [[nodiscard]] auto last_newline() const -> std::pair<size_t, unsigned> { return {last_nl_, nl_}; }
...
template <typename StreamBuffer> class StreamInput {
  public:
    using encoding = typename StreamBuffer::encoding;
    ...
    explicit StreamInput(StreamBuffer &buffer) : buffer_(&buffer) {}

    auto reader() const & { return reader_type{*buffer_, start_}; }
