...
class report_error {
  public:
    ...
    constexpr report_error(Gringo::Logger &log, bool &error, char const *path = nullptr)
        : log_{&log}, error_{&error}, path_{path} {}

    //! Get the corresponding error sink.
    [[nodiscard]] constexpr auto sink() const { return sink_{log_, opts_, error_, path_, 0}; }
