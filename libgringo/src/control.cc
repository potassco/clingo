...
extern "C" clingo_error_t clingo_theory_atoms_term_number(clingo_theory_atoms_t *atoms, clingo_id_t value, int *num) {
}
...
extern "C" clingo_error_t clingo_theory_atoms_atom_type(clingo_theory_atoms_t *atoms, clingo_id_t, clingo_theory_atom_type_t *ret) {
}
...
extern "C" clingo_error_t clingo_control_add(clingo_control_t *ctl, char const *name, char const * const *params, size_t n, char const *part) {
    GRINGO_CLINGO_TRY {
        FWStringVec p;
        for (char const * const *it = params, * const *ie = it + n; it != ie; ++it) { p.emplace_back(*it); }
        ctl->add(name, p, part);
...
TheoryAtomType TheoryAtom::type() const {
    clingo_theory_atom_type_t ret;
    handleError(clingo_theory_atoms_atom_type(atoms_, id_, &ret));
...
SolveIter Control::solve_iter(SymbolicLiteralSpan assumptions) {
    ...
    clingo_control_solve_iter(ctl_, assumptions.begin(), assumptions.size(), &it);
    return it;
}
