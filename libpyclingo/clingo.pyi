# pylint: disable=invalid-name, missing-module-docstring, unused-argument, missing-function-docstring, dangerous-default-value, line-too-long
# pylint: disable=multiple-statements, bad-whitespace, function-redefined, missing-class-docstring, unsubscriptable-object, redefined-builtin

# we can only support python 3.7 but nothing before with this
from __future__ import annotations

from typing import Any, Callable, ContextManager, Iterator, List, Mapping, MutableSequence, Optional, Sequence, Tuple, Union
...
def Tuple_(arguments: List[Symbol]) -> Symbol: ...
...
class Control:
    def __init__(self, arguments: List[str]=[], logger: Callable[[MessageCode,str],None]=None, message_limit: int=20): ...
    ...
    def solve(self, assumptions: List[Union[Tuple[Symbol,bool],int]]=[], on_model: Callable[[Model],Optional[bool]]=None, on_statistics : Callable[[StatisticsMap,StatisticsMap],None]=None, on_finish: Callable[[SolveResult],None]=None, yield_: bool=False, async_: bool=False) -> Union[SolveHandle,SolveResult]: ...
...
class HeuristicType(metaclass=ABCMeta):
...
class MessageCode(metaclass=ABCMeta):
...
class PropagateInit(metaclass=ABCMeta):
    ...
    def add_weight_constraint(self, literal: int, literals: List[Tuple[int,int]], bound: int, type: int=0, compare_equal: bool=False) -> bool: ...
    def propagate(self) -> bool: ...
...
class PropagatorCheckMode(metaclass=ABCMeta):
...
class TheoryTerm(metaclass=ABCMeta):
    arguments: List[TheoryTerm]
    name: str
    number: int
    type: TheoryTermType

class TheoryTermType(metaclass=ABCMeta):
...
class TruthValue(metaclass=ABCMeta):
    False_: TruthValue
    Free: TruthValue
    Release: TruthValue
    True_: TruthValue
